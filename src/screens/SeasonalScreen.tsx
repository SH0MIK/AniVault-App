...
export default function SeasonalScreen() {
  ...
  return <View style={styles.container}>
    ...
    <FlatList data={items} numColumns={3} keyExtractor={x => String(x.id)} contentContainerStyle={styles.grid} columnWrapperStyle={styles.row}
      ...
      renderItem={({ item }) => <View style={styles.card}><WebAnimeCard title={item.title} image={item.image} score={item.score} type={item.type} episodes={item.episodes} status={item.userStatus} width={108} onPress={() => navigation.navigate('AnimeDetail', { id: item.id, title: item.title })}/></View>}/>
  </View>;
}
